...
SQL_GENERATION_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """You are a SQL expert. Generate a SQLite query to answer the user's question.
...
ANSWER_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """You are a helpful assistant answering questions about business data.
...
def get_pre_aggregation_query(sql: str) -> str:
    sql_upper = sql.upper()
    ...
    try:
        from_match = re.search(r'FROM\s+(.+?)(?:\sGROUP BY|\sORDER BY|\sLIMIT|;|$)', sql, re.IGNORECASE | re.DOTALL)
...
def execute_and_answer(question: str) -> dict:
    ...
    logger.info(f"Query analysis: {sql_analysis['table_count']} table(s), {sql_analysis['join_count']} join(s), type: {sql_analysis['query_type']}")
    ...
    try:
        ...
        results_str = results_df.to_string(index=False) if not results_df.empty else "No results found"
