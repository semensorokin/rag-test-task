...
from src.chain import ask, pipeline, get_pipeline_stats
from src.database import init_database, DB_PATH, get_engine
from src.config import TABLE_FILES, LLM_MODEL
...
with tab_chat:
    ...
    chat_container = st.container(height=1300)
...
with tab_stats:
    ...
    if stats.get("last_query"):
        ...
        if lq.get("intermediate"):
            ...
            with st.expander(f"View data before aggregation ({inter['row_count']} rows, {inter['col_count']} columns)", expanded=True):
    ...
    with col_right:
        ...
        if stats["query_history"]:
            ...
            st.line_chart(times, width="stretch")
        else:
